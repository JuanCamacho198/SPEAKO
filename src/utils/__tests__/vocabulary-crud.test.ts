...
describe('vocabulary-storage utilities (additional tests)', () => {
  describe('getVocabularyBoost', () => {
    ...
    it('should filter by language', () => {
      ...
      const boostEn = getVocabularyBoost('hello', 'en', store);
      const boostEs = getVocabularyBoost('hello', 'es', store);
      
      expect(boostEn).toBe(1);
      expect(boostEs).toBe(0);
    });
  });
  ...
  describe('findVocabularyMatches', () => {
    ...
    it('should be case insensitive', () => {
      ...
      const matches = findVocabularyMatches('hello world', 'en', store);
      expect(matches).toHaveLength(1);
    });

    it('should return empty for empty store', () => {
      const store = DEFAULT_VOCABULARY_STORE;
      const matches = findVocabularyMatches('hello world', 'en', store);
      expect(matches).toHaveLength(0);
    });
  });
